...
module.exports = function() {
  ...
  let chartData, cData;
  let margin; // @todo resolve reference;
  ...
  window.buildChart1 = function(staticData, candData) {
    ...
    d3.select("#chart1-title").html(function() {
      let filters = window.currentFilters;
      if (filters.cargo == "deputado-estadual") {
        return (
          "Comparação com os eleitos em 2014 " +
          filters.estado_prep +
          " " +
  ...
  let calcColumnValues = function() {
    ...
    // const svgHeight = colHeight;
    // const svgWidth = colWidth - margin.left - margin.right;
  };
  ...
  let buildAgeChart = function(ageData, age, index) {
    ...
    maxValue = d3.max(ageData, function(d) {
      return d.total;
    });
    allTotals = d3.sum(ageData, function(d) {
      return d.total;
    });
  ...
  let buildEducationChart = function(edData, ed, index) {
    ...
    maxValue = d3.max(edData, function(d) {
      return d.total;
    });
    allTotals = d3.sum(edData, function(d) {
      return d.total;
    });
  ...
  let buildGenderChart = function(gData, property, index) {
    ...
    maxValue = d3.max(gData, function(d) {
      return d.total;
    });

    allTotals = d3.sum(gData, function(d) {
      return d.total;
    });
  ...
  let buildRaceChart = function(ethData, property, index) {
    ...
    maxValue = d3.max(ethData, function(d) {
      return d.total;
    });

    allTotals = d3.sum(ethData, function(d) {
      return d.total;
    });
