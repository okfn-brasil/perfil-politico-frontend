...
module.exports = function() {
  ...
  let setup = function() {
    ...
    d3.select("body").on("click", function() {
      var tooltippedContent = d3.selectAll(
        ".c, .asset-point, .affil-point, .elect-point"
      );
  ...
  window.init = function() {
    ...
    d3.selectAll("#filtro-perguntas option[disabled]").attr("disabled", false);
    ...
    svg = d3
      .select("body")
      .insert("svg", ".first") //d3.select(".block").append("svg")
      .attr("class", "viz")
    ...
    staticURL = [
      "https://api-perfilpolitico.serenata.ai/api/stats/2014/" +
        filters.cargo +
        "/age/",
      "https://api-perfilpolitico.serenata.ai/api/stats/2014/" +
        filters.cargo +
    ...
    staticURLState = [
      "https://api-perfilpolitico.serenata.ai/api/stats/" +
        filters.estado +
        "/2014/" +
        filters.cargo +
        "/age/",
    ...
    candidates
      ...
      .on("mouseover", function(d) {
        d3.select("#info-box").html(
          window.capitalizeName(d.name) + " (" + d.party + ")"
        );
        d3.select("#info-box").classed("hidden", false);
  ...
  let fillCandidateInfo = function(cData) {
    ...
    d3.select("#profile-name span").text(
      window.capitalizeName(cData.ballot_name) +
        " (" +
        cData.ballot_number +
        ")"
    );
    ...
    d3.select("#info-profissao").text(() => {
      ...
      if (cData.gender.toLowerCase() == "feminino") {
        var gender_translate = require("../data/jobs.js");
        var temp = gender_translate[cData.occupation.toUpperCase()];
        if (temp != null) {
          return_str = temp.toLowerCase();
        }
      }
      return return_str;
    });
  ...
  let fillRosiesSuspicions = function(suspicions) {
    ...
    if (suspicions.length > 1) {
      var total = suspicions.reduce(function(total, suspicion) {
        return total + suspicion.value;
      }, 0);
      var summary = document.createElement("p");
      var summaryLabel = document.createElement("b");
      var summaryText = document.createTextNode(
        " " + suspicions.length + " suspeitas / " + toCurrency(total)
      );
  ...
  let disableQuestion = function(type) {
    ...
    let questionsCustom = document.querySelectorAll(
      "#questions .select-items div"
    );
  ...
  let animateParties = function(lastTime) {
    ...
    let bar = bars.filter(function(d, i) {
      return i == selectedBar;
    });
...
const exceptions = ["Do", "Da", "Dos", "Das", "De", "Des", "Van", "Von", "E"];
